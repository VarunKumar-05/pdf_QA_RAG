...
#                                 "mode": mode,
#                                 "collection_name": st.session_state.collection_name_combine  
...
#                             st.rerun()
#                         else:
#                             st.error("Failed to remove PDF embeddings from Qdrant.")
#                     except Exception as e:
...
#     main()
    ...
    # pdf -remove -X mark from the upload function                  - can't be done
    # remove the empty collections from mongoDB - from backend      - done
    #set the name fro separate collections in qdrant and mongoDB
    #search button fro both QG and ask questions                    - done
    #qg-display message if no context to the pdf
    # y reload everytime the user someother buttons                 - done 

    # if st.session_state.pdf_collections:
    #     remove_pdf = st.selectbox(
    #         "Remove a PDF (removes embeddings too):",
    #         ["None"] + [f"{d['filename']} ({d['hash'][:8]})" for d in st.session_state.pdf_collections],
    #         key="remove_pdf_selector"
    #     )
    #     if remove_pdf and remove_pdf != "None":
    #         idx = [f"{d['filename']} ({d['hash'][:8]})" for d in st.session_state.pdf_collections].index(remove_pdf)
    #         pdf_to_remove = st.session_state.pdf_collections[idx]
    #         if st.button(f"Remove {remove_pdf}"):
    #             response = requests.post(
    #                 f"{BACKEND_URL}/remove-pdf",
    #                 json={
    #                     "pdf_hash": pdf_to_remove["hash"],
    #                     "filename": pdf_to_remove["filename"],
    #                     "mode": mode
    #                 }
    #             )
    #             if response.status_code == 200:
    #                 st.success(f"Removed {pdf_to_remove['filename']} from Qdrant!")
    #                 del st.session_state.pdf_collections[idx]
    #             else:
    #                 st.error("Failed to remove PDF embeddings from Qdrant.")
